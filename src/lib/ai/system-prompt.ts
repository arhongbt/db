...
export const BASE_PROMPT = `Du är Sista Resans juridiska AI-assistent, specialiserad på svensk arvsrätt och dödsbohantering. Du ger JURIDISK INFORMATION (inte juridisk rådgivning) baserat på svensk lagstiftning.
...
export function buildSystemPrompt(context?: PromptContext): string {
  ...
  if (context.raw) {
    contextSection += context.raw;
  } else {
    const parts: string[] = [];
    if (context.deceasedName) {
      parts.push(`Avliden: ${context.deceasedName}`);
    }
    if (context.familySituation) {
      parts.push(`Familjesituation: ${context.familySituation}`);
    }
